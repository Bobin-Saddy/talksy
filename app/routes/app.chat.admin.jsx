...
export default function NeuralChatAdmin() {
          ...
          if (latestMsg.id !== lastMessageIdRef.current) {
            ...
            if (latestMsg.sender === "user" && !isFirstLoadRef.current) {
              ...
              if (Notification.permission === "granted") {
                try {
                  new Notification(`New message from ${activeSession.email}`, { 
  ...
  const loadChat = async (session) => {
    ...
    isFirstLoadRef.current = true; // Reset on new chat load
    fetchUserLocation();
    ...
    try {
      ...
      setMessages(data);
      // After initial load, mark as not first load
      setTimeout(() => {
        isFirstLoadRef.current = false;
      }, 500);
    } catch (err) {
  ...
  return (
    <div style={{ display: 'flex', height: 'calc(100vh - 40px)', width: 'calc(100vw - 40px)', backgroundColor: '#fff', margin: '20px', borderRadius: '24px', overflow: 'hidden', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.1)', border: '1px solid #eee', color: '#433d3c', fontFamily: '"Plus Jakarta Sans", sans-serif' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', background: '#fff' }}>
        {activeSession ? (
                ...
                return (
                  <div key={msg.id || i} style={{ alignSelf: isAdmin ? 'flex-end' : 'flex-start', maxWidth: '70%' }}>
                    <div style={{ 
                      ...
                      {isImg ? (
                        <img src={msg.fileUrl} onClick={() => setSelectedImage(msg.fileUrl)} style={{ maxWidth: '100%', borderRadius: '14px', cursor: 'zoom-in' }} alt="Attachment" />
                      ) : isPdf ? (
                        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                          <Icons.FileText /> <a href={msg.fileUrl} download style={{ color: isAdmin ? '#fff' : accentColor, textDecoration: 'none' }}>Download PDF</a>
                        </div>
                      ) : (
                        <div style={{ fontSize: '15px', wordWrap: 'break-word' }}>{msg.message}</div>
                      )}
                    </div>
                  </div>
                );
            ...
            <div style={{ padding: '30px 40px', background: '#fff', borderTop: '1px solid #f0f0f0', position: 'relative' }}>
              {showEmojiPicker && (
                <div style={{ position: 'absolute', bottom: '100px', left: '40px', background: '#fff', padding: '16px', borderRadius: '24px', boxShadow: '0 20px 50px rgba(0,0,0,0.15)', display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: '12px', zIndex: 100 }}>
                  {emojis.map(e => (
                    <button key={e} onClick={() => { setReply(r => r + e); setShowEmojiPicker(false); }} style={{ fontSize: '22px', border: 'none', background: 'none', cursor: 'pointer', padding: '8px' }}>{e}</button>
                  ))}
                </div>
              )}
              <div style={{ display: 'flex', alignItems: 'center', background: '#f8f7f6', borderRadius: '20px', padding: '8px 10px', border: '1px solid #eee' }}>
                ...
                <button onClick={() => setShowEmojiPicker(!showEmojiPicker)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#a8a29e', padding: '8px' }}><Icons.Smile /></button>
                ...
                <input placeholder="Type your response..." style={{ flex: 1, border: 'none', background: 'transparent', outline: 'none', fontSize: '15px', padding: '0 15px' }} value={reply} onChange={(e) => setReply(e.target.value)} onKeyPress={(e) => e.key === 'Enter' && !e.shiftKey && handleReply()} />
                <button onClick={() => handleReply()} disabled={!reply.trim()} style={{ width: '48px', height: '48px', borderRadius: '16px', background: reply.trim() ? accentColor : '#e5e7eb', border: 'none', color: 'white', cursor: reply.trim() ? 'pointer' : 'not-allowed', transition: 'all 0.2s' }}><Icons.Send /></button>
              </div>
            </div>
          </>
        ) : (
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', color: '#d1cfcd' }}>
            <Icons.User size={100} />
            <p style={{ marginTop: '20px', fontSize: '16px' }}>Select a chat to begin</p>
          </div>
        )}
      </div>
      ...
      <div style={{ width: '340px', padding: '32px 24px', background: '#fff', borderLeft: '1px solid #f0f0f0' }}>
        <h4 style={{ fontSize: '12px', fontWeight: '900', color: '#a8a29e', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '20px' }}>Intelligence Hub</h4>
        {activeSession ? (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            <div style={{ padding: '24px', background: '#f0f9ff', borderRadius: '24px' }}>
              ...
              <div style={{ display: 'flex', gap: '15px', alignItems: 'center' }}>
                {liveLocation.flag && <img src={liveLocation.flag} width="35" style={{ borderRadius: '4px' }} alt="Flag" />}
