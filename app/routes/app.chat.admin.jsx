...
export default function NeuralChatAdmin() {
  ...
  const messagesRef = useRef([]); // ✅ KEY FIX: prevents stale closure in polling

  const emojis = ["😊", "👍", "❤️", "🙌", "✨", "🔥", "✅", "🤔", "💡", "🚀", "👋", "🙏", "🎉"];

  // ── Init audio + notifications ──
  useEffect(() => {
  ...
  // ────────────────────────────────────────────────────────────────
  // SESSION POLLING — 1.5s, detects new sessions & updated sessions
  // ────────────────────────────────────────────────────────────────
  useEffect(() => {
  ...
  cache: "no-store"
        ...
        const currentIds = current.map(s => s.sessionId);

        const newSessions = data.sessions.filter(s => !currentIds.includes(s.sessionId));
        const updatedSessions = data.sessions.filter(newS => {
          const old = current.find(s => s.sessionId === newS.sessionId);
          return old && new Date(newS.updatedAt) > new Date(old.updatedAt);
        });

        const hasChanges =
          data.sessions.length !== current.length ||
  ...
  }, []); // empty dep — sessionsRef avoids stale closure

  // ────────────────────────────────────────────────────────────────
  // MESSAGE POLLING — 1.5s when a chat is open
  // ────────────────────────────────────────────────────────────────
  useEffect(() => {
    ...
    const pollMessages = async () => {
      try {
        ...
        // ✅ Use ref — NOT stale `messages` state
        const current = messagesRef.current;
        const hasChanges =
          data.length !== current.length ||
          (data.length > 0 && current.length > 0 &&
        ...
        if (hasChanges) {
          if (data.length > 0) {
            ...
            const isNewUserMsg =
              latest.sender === "user" &&
              String(latest.id) !== String(lastMessageIdRef.current) &&
              !isFirstLoadRef.current;

            if (isNewUserMsg) notifyNewMessage(activeSession, latest);
  ...
  const filteredSessions = useMemo(() => {
    ...
    if (filterStatus === "requests") {
      list = list.filter(s => s.isOverLimit === true);
    } else {
      list = list.filter(s => s.isOverLimit !== true);
      if (filterStatus === "pending")  list = list.filter(s => !s.isResolved);
      if (filterStatus === "resolved") list = list.filter(s => s.isResolved);
    }
    return list;
  ...
  // ✅ MERGED: optimistic UI (from old file) + async fetch with error rollback (from new file)
  const handleReply = async (text = null) => {
    if (isActiveSessionOverLimit) {
      alert("This chat is over your plan limit. Upgrade to respond.");
      return;
    }
    ...
    try {
      ...
      // On success: polling will replace tempId with real server message
    } catch (error) {
      setMessages(prev => prev.filter(m => m.id !== tempId));
      alert("Failed to send message. Please try again.");
    }
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', backgroundColor: '#f9fafb', color: '#111827', fontFamily: '"Inter", system-ui, sans-serif' }}>
      ...
      <div style={{ width: '360px', borderRight: '1px solid #e5e7eb', display: 'flex', flexDirection: 'column', background: '#fff' }}>
        ...
        {planLimit.isOverLimit && (
          <div style={{ margin: '0 16px 16px', padding: '14px', background: 'linear-gradient(135deg, #7c3aed, #6366f1)', borderRadius: '12px', color: 'white', boxShadow: '0 4px 12px rgba(124,58,237,0.3)' }}>
            ...
            <button onClick={() => window.location.href = '/app/subscription'} style={{ marginTop: '10px', padding: '8px 14px', background: 'white', color: '#7c3aed', border: 'none', borderRadius: '8px', fontWeight: '600', fontSize: '12px', cursor: 'pointer', width: '100%' }}>
              Upgrade to Continue
            </button>
          </div>
        )}
        ...
        <div style={{ padding: '0 16px 16px', display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: '6px', borderBottom: '1px solid #f3f4f6' }}>
          {[
            { key: 'all',      label: 'All',      color: '#6366f1', count: withinLimitSessions.length },
            { key: 'pending',  label: 'Pending',  color: '#f59e0b', count: withinLimitSessions.filter(s => !s.isResolved).length },
            { key: 'resolved', label: 'Resolved', color: '#10b981', count: withinLimitSessions.filter(s => s.isResolved).length },
          ].map(({ key, label, color, count }) => (
            <button key={key} onClick={() => setFilterStatus(key)} style={{ padding: '8px 12px', borderRadius: '8px', border: filterStatus === key ? 'none' : '1px solid #e5e7eb', background: filterStatus === key ? color : '#fff', color: filterStatus === key ? '#fff' : '#6b7280', fontWeight: '600', fontSize: '12px', cursor: 'pointer', transition: 'all 0.2s' }}>
              {label} {count > 0 && `(${count})`}
            </button>
          ))}
          <button onClick={() => setFilterStatus("requests")} style={{ padding: '8px 12px', borderRadius: '8px', border: filterStatus === "requests" ? 'none' : '1px solid #e5e7eb', background: filterStatus === "requests" ? '#7c3aed' : '#fff', color: filterStatus === "requests" ? '#fff' : '#6b7280', fontWeight: '600', fontSize: '12px', cursor: 'pointer', transition: 'all 0.2s', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '4px' }}>
            <Icons.Inbox />
        ...
        <div style={{ padding: '16px' }}>
          <div style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
            ...
            <input placeholder="Search conversations..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} style={{ width: '100%', padding: '10px 10px 10px 40px', borderRadius: '10px', border: '1px solid #e5e7eb', outline: 'none', fontSize: '14px' }} />
          </div>
        </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '0 12px' }}>
          {filteredSessions.map(session => (
            <div key={session.sessionId} onClick={() => loadChat(session)} style={{ padding: '12px', borderRadius: '12px', cursor: 'pointer', marginBottom: '6px', background: activeSession?.sessionId === session.sessionId ? '#f0f9ff' : 'transparent', border: activeSession?.sessionId === session.sessionId ? '1px solid #bae6fd' : '1px solid transparent', transition: 'all 0.2s' }}>
              <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
                <div style={{ width: '44px', height: '44px', borderRadius: '12px', background: activeSession?.sessionId === session.sessionId ? 'linear-gradient(135deg, #6366f1, #8b5cf6)' : (session.isOverLimit ? '#fef3c7' : '#f3f4f6'), display: 'flex', alignItems: 'center', justifyContent: 'center', color: activeSession?.sessionId === session.sessionId ? 'white' : (session.isOverLimit ? '#92400e' : '#9ca3af'), flexShrink: 0 }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: 2 }}>
                    ...
                    {session.isOverLimit && <div style={{ background: '#f59e0b', color: 'white', fontSize: '9px', fontWeight: '700', padding: '2px 6px', borderRadius: '4px' }}>REQUEST</div>}
                  </div>
            ...
            <div style={{ padding: '20px 32px', borderBottom: '1px solid #e5e7eb', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: isActiveSessionOverLimit ? '#fef3c7' : '#fff' }}>
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                  ...
                  {isActiveSessionOverLimit && <div style={{ background: '#f59e0b', color: 'white', fontSize: '11px', fontWeight: '700', padding: '4px 10px', borderRadius: '6px', display: 'flex', alignItems: 'center', gap: '4px' }}><Icons.Lock />Over Limit</div>}
                  {activeSession.isResolved && !isActiveSessionOverLimit && <div style={{ background: '#d1fae5', color: '#065f46', fontSize: '11px', fontWeight: '700', padding: '4px 10px', borderRadius: '6px', display: 'flex', alignItems: 'center', gap: '4px' }}><Icons.Check />Resolved</div>}
                </div>
                {isActiveSessionOverLimit && <div style={{ fontSize: '12px', color: '#92400e', marginTop: '4px', fontWeight: '500' }}>This chat is over your plan limit. Upgrade to respond.</div>}
                {activeSession.resolvedAt && !isActiveSessionOverLimit && <div style={{ fontSize: '12px', color: '#6b7280', marginTop: '4px' }}>Resolved on {new Date(activeSession.resolvedAt).toLocaleDateString()}</div>}
              </div>
              {!isActiveSessionOverLimit && !activeSession.isResolved && (
                <button onClick={handleMarkResolved} style={{ padding: '10px 18px', borderRadius: '10px', border: 'none', background: 'linear-gradient(135deg, #10b981, #059669)', color: '#fff', fontWeight: '600', fontSize: '13px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px', boxShadow: '0 4px 12px rgba(16,185,129,0.25)', transition: 'all 0.2s' }} onMouseEnter={e => e.currentTarget.style.transform = 'translateY(-1px)'} onMouseLeave={e => e.currentTarget.style.transform = 'translateY(0)'}>
                  <Icons.CheckCircle />Mark as Resolved
                </button>
              )}
              {!isActiveSessionOverLimit && activeSession.isResolved && (
                <button onClick={handleReopenChat} style={{ padding: '10px 18px', borderRadius: '10px', border: '2px solid #f59e0b', background: '#fff', color: '#f59e0b', fontWeight: '600', fontSize: '13px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px', transition: 'all 0.2s' }} onMouseEnter={e => e.currentTarget.style.background = '#fffbeb'} onMouseLeave={e => e.currentTarget.style.background = '#fff'}>
              ...
              {messages.map((msg, i) => (
                <div key={msg.id || i} style={{ alignSelf: msg.sender === 'admin' ? 'flex-end' : 'flex-start', maxWidth: '65%' }}>
                  <div style={{ padding: '12px 16px', borderRadius: '16px', background: msg.sender === 'admin' ? 'linear-gradient(135deg, #6366f1, #8b5cf6)' : (msg.sender === 'bot' ? '#fef3c7' : '#fff'), color: msg.sender === 'admin' ? '#fff' : '#111827', boxShadow: '0 2px 8px rgba(0,0,0,0.06)', border: msg.sender === 'admin' ? 'none' : (msg.sender === 'bot' ? '1px solid #fbbf24' : '1px solid #e5e7eb'),
                    // ✅ Dim optimistic messages until confirmed
                    opacity: String(msg.id).startsWith('temp-') ? 0.6 : 1, transition: 'opacity 0.3s'
                  }}>
                    {msg.fileUrl
                  ...
                  <div style={{ fontSize: '11px', color: '#9ca3af', marginTop: '4px', textAlign: msg.sender === 'admin' ? 'right' : 'left' }}>
                    {String(msg.id).startsWith('temp-') ? 'Sending…' : new Date(msg.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                  </div>
                </div>
              ))}
            </div>
              ...
              <div style={{ padding: '20px 32px', background: '#fff', borderTop: '1px solid #e5e7eb', position: 'relative' }}>
                {showEmojiPicker && (
                  <div style={{ position: 'absolute', bottom: '80px', left: '32px', background: 'white', padding: '12px', borderRadius: '12px', boxShadow: '0 10px 25px rgba(0,0,0,0.15)', border: '1px solid #e5e7eb', display: 'grid', gridTemplateColumns: 'repeat(5,1fr)', gap: '8px', zIndex: 10 }}>
                    {emojis.map(e => (
                      <button key={e} onClick={() => addEmoji(e)} style={{ background: 'none', border: 'none', fontSize: '22px', cursor: 'pointer', padding: '6px', borderRadius: '8px' }} onMouseEnter={ev => ev.target.style.background = '#f3f4f6'} onMouseLeave={ev => ev.target.style.background = 'none'}>{e}</button>
                    ))}
                  </div>
                )}
                <div style={{ display: 'flex', alignItems: 'center', background: '#f9fafb', borderRadius: '12px', padding: '6px 8px', border: '1px solid #e5e7eb' }}>
                  ...
                  <input placeholder="Type a message..." value={reply} onChange={e => setReply(e.target.value)} onKeyPress={e => { if (e.key === 'Enter') handleReply(); }} style={{ flex: 1, border: 'none', background: 'transparent', outline: 'none', fontSize: '14px', color: '#111827' }} />
                  <button onClick={() => handleReply()} style={{ width: '40px', height: '40px', borderRadius: '10px', background: (reply.trim() || filePreview) ? 'linear-gradient(135deg, #6366f1, #8b5cf6)' : '#e5e7eb', border: 'none', color: 'white', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.2s' }}><Icons.Send /></button>
                </div>
              </div>
            ) : (
      ...
      <div style={{ width: '320px', padding: '24px', background: '#fff', borderLeft: '1px solid #e5e7eb' }}>
        ...
        <div style={{ marginBottom: '16px', padding: '16px', background: planLimit.isOverLimit ? '#fef3c7' : planLimit.isAtLimit ? '#fee2e2' : planLimit.isNearLimit ? '#fef3c7' : '#eff6ff', borderRadius: '12px', border: `1px solid ${planLimit.isOverLimit ? '#fcd34d' : planLimit.isAtLimit ? '#fca5a5' : planLimit.isNearLimit ? '#fcd34d' : '#bfdbfe'}` }}>
          <div style={{ fontSize: '10px', color: planLimit.isOverLimit || planLimit.isNearLimit ? '#92400e' : planLimit.isAtLimit ? '#991b1b' : '#1e40af', fontWeight: '700', marginBottom: '8px', letterSpacing: '0.5px' }}>PLAN USAGE</div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
            <Icons.TrendingUp />
            <span style={{ fontSize: '24px', fontWeight: '800', color: planLimit.isOverLimit || planLimit.isNearLimit ? '#92400e' : planLimit.isAtLimit ? '#991b1b' : '#1e40af' }}>
              {planLimit.max > 0 ? `${withinLimitSessions.length}/${planLimit.max}` : sessions.length}
            </span>
          </div>
          <div style={{ fontSize: '11px', color: planLimit.isOverLimit ? '#92400e' : planLimit.isAtLimit ? '#991b1b' : '#60a5fa' }}>
            {planLimit.isOverLimit
              ? `${planLimit.overLimitBy} request${planLimit.overLimitBy !== 1 ? 's' : ''} in queue`
              : planLimit.isAtLimit ? 'At limit - upgrade to continue'
        ...
        {activeSession && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            <div style={{ padding: '16px', background: activeSession.isResolved ? '#d1fae5' : isActiveSessionOverLimit ? '#fee2e2' : '#fef3c7', borderRadius: '12px', border: `1px solid ${activeSession.isResolved ? '#86efac' : isActiveSessionOverLimit ? '#fca5a5' : '#fcd34d'}` }}>
