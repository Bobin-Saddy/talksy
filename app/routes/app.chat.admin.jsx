...
//  FCM REMOVED from this file — now handled globally in app.jsx
//  This file only handles chat UI, polling, and session management.
//  Notifications work on ALL pages because app.jsx owns FCM setup.
// ═══════════════════════════════════════════════════════════
...
// ─── VAPID push helpers (legacy toggle button) ────────────
...
export default function NeuralChatAdmin() {
  ...
  // ✅ Notification permission state — for "Enable Notifications" UI only
  // FCM setup itself is done in app.jsx globally
  const [notifPermission, setNotifPermission] = useState("default");
  const [showPermissionGuide, setShowPermissionGuide] = useState(false);

  // ✅ Local toasts for this page (clicking opens the chat session)
  ...
  const pushToast = (title, body, sessionId, imageUrl = null) => {
    ...
    setAdminToasts(prev => [...prev, {
      id: toastId, title, body, sessionId, imageUrl,
      isImage: !!(imageUrl),
      time: new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit", second: "2-digit" }),
    }]);
    setTimeout(() => setAdminToasts(prev => prev.filter(t => t.id !== toastId)), 8000);
  };

  // ── Init audio, read permission state ─────────────────────
  useEffect(() => {
  ...
  // This is the ONLY place where we prompt for permission.
  // Once granted, the global FCM setup in app.jsx takes over automatically
  // on the next page load (or immediately if we trigger it via window event).
  const handleEnableNotifications = async () => {
    ...
    if (perm === "granted") {
      // ✅ Dispatch custom event so app.jsx FCM setup reruns immediately
      // without waiting for next page load
      window.dispatchEvent(new Event("talksy:permission-granted"));
    }
  };

  // ── VAPID push toggle (legacy) ────────────────────────────
  const handlePushToggle = async () => {
    setPushLoading(true);
    try {
      if (pushEnabled) {
        const ok = await unsubscribeFromPush(currentShop);
        if (ok) { setPushEnabled(false); alert("🔕 Push notifications disabled."); }
      } else {
        const sub = await subscribeToPush(currentShop);
        if (sub) {
          setPushEnabled(true);
          alert("✅ Push notifications enabled!\nYou will now receive browser notifications when customers send messages.");
        }
      }
    } finally {
  ...
  return (
    <div style={{ display:"flex", height:"100vh", backgroundColor:"#f9fafb", color:"#111827", fontFamily:'"Inter", system-ui, sans-serif' }}>
      ...
      {/* ── Toast stack (chat-page toasts with "tap to open" functionality) ── */}
      {adminToasts.length > 0 && (
        <div style={{ position:"fixed", bottom:"32px", right:"32px", zIndex:99999, display:"flex", flexDirection:"column-reverse", gap:"10px", maxWidth:"360px", width:"360px", pointerEvents:"none" }}>
          {adminToasts.length >= 2 && (
            <div style={{ pointerEvents:"auto", display:"flex", justifyContent:"flex-end" }}>
              <button onClick={() => setAdminToasts([])} style={{ padding:"5px 12px", background:"rgba(0,0,0,0.55)", color:"white", border:"none", borderRadius:"20px", fontSize:"11px", fontWeight:"600", cursor:"pointer", backdropFilter:"blur(4px)" }}>
          ...
          {adminToasts.map((toast, idx) => (
            <div
              ...
              style={{ pointerEvents:"auto", background:"linear-gradient(135deg, #6366f1 0%, #8b5cf6 100%)", color:"white", borderRadius:"16px", padding:"14px 16px", boxShadow:"0 6px 24px rgba(99,102,241,0.45)", display:"flex", alignItems:"flex-start", gap:"12px", cursor:toast.sessionId?"pointer":"default", animation:"slideInRight 0.3s ease", border:"1px solid rgba(255,255,255,0.15)", position:"relative", overflow:"hidden" }}
            >
              <div style={{ position:"absolute", bottom:0, left:0, height:"3px", background:"rgba(255,255,255,0.4)", borderRadius:"0 0 0 16px", animation:"toastProgress 7s linear forwards", width:"100%" }} />
              <div style={{ position:"relative", flexShrink:0 }}>
                <div style={{ width:"38px", height:"38px", background:"rgba(255,255,255,0.2)", borderRadius:"10px", display:"flex", alignItems:"center", justifyContent:"center", fontSize:"18px" }}>💬</div>
                {idx === 0 && adminToasts.length > 1 && (
                  <div style={{ position:"absolute", top:"-6px", right:"-6px", background:"#ef4444", color:"white", fontSize:"9px", fontWeight:"800", width:"16px", height:"16px", borderRadius:"50%", display:"flex", alignItems:"center", justifyContent:"center", border:"2px solid white" }}>{adminToasts.length}</div>
                )}
              </div>
              <div style={{ flex:1, minWidth:0 }}>
                <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:"4px" }}>
                  <div style={{ fontWeight:"700", fontSize:"13px", whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis", maxWidth:"200px" }}>{toast.title}</div>
                ...
                {toast.imageUrl && (
                  <div style={{ marginBottom:"6px", borderRadius:"8px", overflow:"hidden", maxHeight:"120px", background:"rgba(0,0,0,0.2)" }}>
                    <img src={toast.imageUrl} alt="attachment" style={{ width:"100%", maxHeight:"120px", objectFit:"cover", borderRadius:"8px", display:"block" }} onError={e => { e.currentTarget.style.display = "none"; }} />
                  </div>
                )}
                <div style={{ fontSize:"12px", opacity:0.9, lineHeight:"1.45", wordBreak:"break-word" }}>{toast.body}</div>
                {toast.sessionId && <div style={{ fontSize:"10px", opacity:0.65, marginTop:"5px", display:"flex", alignItems:"center", gap:"4px" }}><span>👆</span> Tap to open chat</div>}
              </div>
              <button onClick={e => { e.stopPropagation(); setAdminToasts(prev => prev.filter(t => t.id !== toast.id)); }} style={{ background:"rgba(255,255,255,0.2)", border:"none", color:"white", width:"22px", height:"22px", borderRadius:"6px", cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0, fontSize:"13px", fontWeight:"700", marginTop:"1px" }}>×</button>
            </div>
          ))}
        </div>
      )}
      ...
      {showBlurPopup && (
        <div onClick={() => setShowBlurPopup(false)} style={{ position:"fixed", top:0, left:0, width:"100vw", height:"100vh", backgroundColor:"rgba(0,0,0,0.7)", zIndex:10000, display:"flex", alignItems:"center", justifyContent:"center", backdropFilter:"blur(8px)", cursor:"pointer" }}>
          <div onClick={e => e.stopPropagation()} style={{ background:"white", borderRadius:"20px", padding:"40px", maxWidth:"500px", boxShadow:"0 20px 60px rgba(0,0,0,0.3)", cursor:"default", position:"relative" }}>
            <button onClick={() => setShowBlurPopup(false)} style={{ position:"absolute", top:"16px", right:"16px", background:"none", border:"none", cursor:"pointer", padding:"8px", borderRadius:"8px", display:"flex" }}><Icons.X size={20} color="#9ca3af" /></button>
      ...
      <div style={{ width:"360px", borderRight:"1px solid #e5e7eb", display:"flex", flexDirection:"column", background:"#fff" }}>
        ...
        {showPermissionGuide && (
          <div onClick={() => setShowPermissionGuide(false)} style={{ position:"fixed", top:0, left:0, width:"100vw", height:"100vh", background:"rgba(0,0,0,0.65)", zIndex:99998, display:"flex", alignItems:"center", justifyContent:"center", backdropFilter:"blur(6px)" }}>
            <div onClick={e => e.stopPropagation()} style={{ background:"white", borderRadius:"20px", padding:"28px 32px", maxWidth:"440px", width:"92%", boxShadow:"0 24px 64px rgba(0,0,0,0.3)", position:"relative" }}>
              <button onClick={() => setShowPermissionGuide(false)} style={{ position:"absolute", top:"14px", right:"14px", background:"#f3f4f6", border:"none", borderRadius:"8px", width:"30px", height:"30px", cursor:"pointer", fontSize:"16px", display:"flex", alignItems:"center", justifyContent:"center", color:"#6b7280" }}>×</button>
              <div style={{ textAlign:"center", marginBottom:"20px" }}>
                <div style={{ fontSize:"44px", marginBottom:"10px" }}>🚫</div>
                <h2 style={{ margin:0, fontSize:"19px", fontWeight:"800", color:"#dc2626" }}>Notifications Blocked</h2>
                <p style={{ margin:"8px 0 0", fontSize:"13px", color:"#6b7280", lineHeight:"1.6" }}>You previously blocked notifications. Follow these steps to fix:</p>
              </div>
              <div style={{ display:"flex", flexDirection:"column", gap:"10px", marginBottom:"20px" }}>
                {[
                  { n:1, title:"🌐 Open Chrome Notification Settings", sub:"Open a new Chrome tab, paste this URL and press Enter:", code:"chrome://settings/content/notifications", bg:"#f3f4f6", color:"#111827" },
                  { n:2, title:"🗑️ Delete the block for Talksy", sub:'Under "Not allowed", find this site and click the trash icon:', code:"talksy-production-5d43.up.railway.app", bg:"#fee2e2", color:"#991b1b" },
                  { n:3, title:"✅ Return here and click Retry", sub:"After deleting the block, come back to this tab and click the button below.", code:null, bg:null, color:null, green:true },
                ].map(step => (
                  <div key={step.n} style={{ background:"#f9fafb", border:`1px solid ${step.green?"#bbf7d0":"#e5e7eb"}`, borderRadius:"10px", padding:"12px 14px", display:"flex", gap:"12px" }}>
                    <div style={{ width:"24px", height:"24px", borderRadius:"50%", background:step.green?"#10b981":"#6366f1", color:"white", fontSize:"12px", fontWeight:"800", display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>{step.n}</div>
                    <div style={{ flex:1 }}>
                      <div style={{ fontWeight:"700", fontSize:"12px", color:step.green?"#065f46":"#111827", marginBottom:"4px" }}>{step.title}</div>
                      <div style={{ fontSize:"11px", color:"#6b7280", marginBottom: step.code?"8px":"0" }}>{step.sub}</div>
                      {step.code && (
                        <div style={{ display:"flex", gap:"6px", alignItems:"center" }}>
                          <code style={{ flex:1, background:step.bg, padding:"5px 8px", borderRadius:"5px", fontSize:"10px", color:step.color, fontFamily:"monospace", wordBreak:"break-all" }}>{step.code}</code>
                          <button onClick={() => navigator.clipboard.writeText(step.code).catch(() => {})} style={{ flexShrink:0, padding:"5px 8px", background:step.green?"#10b981":step.n===2?"#ef4444":"#6366f1", color:"white", border:"none", borderRadius:"5px", fontSize:"10px", fontWeight:"700", cursor:"pointer" }}>Copy</button>
                        </div>
                      )}
                    </div>
                  </div>
              ...
              <button
                ...
                style={{ width:"100%", padding:"13px", background:"linear-gradient(135deg, #ea580c 0%, #dc2626 100%)", color:"white", border:"none", borderRadius:"12px", fontWeight:"800", fontSize:"14px", cursor:"pointer", boxShadow:"0 4px 14px rgba(220,38,38,0.35)" }}
              >
        ...
        {notifPermission !== "granted" && (
          <div style={{ margin:"0 16px 8px", padding:"12px 14px", background:notifPermission==="denied"?"linear-gradient(135deg, #fff7ed, #fef2f2)":"linear-gradient(135deg, #eff6ff, #f5f3ff)", border:notifPermission==="denied"?"1px solid #fca5a5":"1px solid #c7d2fe", borderRadius:"12px" }}>
            ...
            <button
              onClick={async () => {
                if (notifPermission === "denied") { setShowPermissionGuide(true); }
                else { await handleEnableNotifications(); }
              }}
              style={{ width:"100%", padding:"9px", background:notifPermission==="denied"?"linear-gradient(135deg, #ea580c 0%, #dc2626 100%)":"linear-gradient(135deg, #6366f1 0%, #8b5cf6 100%)", color:"white", border:"none", borderRadius:"8px", fontWeight:"700", fontSize:"12px", cursor:"pointer", boxShadow:"0 2px 8px rgba(99,102,241,0.3)" }}
            >
        ...
        {planLimit.isOverLimit && (
          <div style={{ margin:"0 16px 16px", padding:"14px", background:"linear-gradient(135deg, #7c3aed 0%, #6366f1 100%)", borderRadius:"12px", color:"white", boxShadow:"0 4px 12px rgba(124,58,237,0.3)" }}>
            <div style={{ display:"flex", alignItems:"center", gap:"10px", marginBottom:"8px" }}><Icons.AlertTriangle /><div style={{ fontSize:"13px", fontWeight:"700" }}>Over Plan Limit (+{planLimit.overLimitBy} extra)</div></div>
            ...
            <div key={session.sessionId} onClick={() => loadChat(session)} style={{ position:"relative", padding:"12px", borderRadius:"12px", cursor:"pointer", marginBottom:"6px", background:activeSession?.sessionId===session.sessionId?"#f0f9ff":"transparent", border:activeSession?.sessionId===session.sessionId?"1px solid #bae6fd":"1px solid transparent", transition:"all 0.2s", opacity:session.shouldBlur?0.6:1 }}>
              <div style={{ display:"flex", gap:"12px", alignItems:"center" }}>
                ...
                {unreadCounts[session.sessionId] > 0 && (
                  <div style={{ background:"#ef4444", color:"white", fontSize:"10px", fontWeight:"700", padding:"4px 8px", borderRadius:"10px", minWidth:"20px", textAlign:"center" }}>{unreadCounts[session.sessionId]}</div>
                )}
              </div>
            </div>
          ))}
