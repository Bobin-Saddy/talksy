...
export default function NeuralChatAdmin() {
  ...
  // --- LOGIC: Newest Message Session always on top ---
  ...
  // --- POLLING: Fetch sessions to detect new messages and sort ---
  useEffect(() => {
    const interval = setInterval(async () => {
      try {
        ...
        setSessions(prev => {
            updatedSessions.forEach(newS => {
                const oldS = prev.find(p => p.sessionId === newS.sessionId);
                if(newS.messages[0]?.id !== oldS?.messages[0]?.id && newS.messages[0]?.sender === 'user') {
                    if(activeSession?.sessionId !== newS.sessionId) {
                        setUnreadCounts(u => ({...u, [newS.sessionId]: (u[newS.sessionId] || 0) + 1}));
  ...
  const handleReply = (text = null) => {
    ...
    if ((!finalMsg.trim() && !filePreview) || !activeSession) return;
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '0 16px' }}>
          {sortedSessions
            ...
            <div key={session.sessionId} onClick={() => loadChat(session)} style={{ 
                ...
                border: unreadCounts[session.sessionId] > 0 ? '1px solid #ef4444' : (activeSession?.sessionId === session.sessionId ? '1px solid #f0f0f0' : '1px solid transparent'),
                transition: 'all 0.2s' 
            }}>
              <div style={{ display: 'flex', gap: '14px', alignItems: 'center' }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontWeight: unreadCounts[session.sessionId] > 0 ? '900' : '700', fontSize: '15px' }}>{session.email?.split('@')[0] || 'User'}</div>
                  <div style={{ fontSize: '13px', color: unreadCounts[session.sessionId] > 0 ? '#1a1615' : '#78716c', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    {session.messages[0]?.message || 'New Chat'}
                  </div>
                </div>
                {unreadCounts[session.sessionId] > 0 && (
                  <div style={{ background: '#ef4444', color: 'white', fontSize: '11px', fontWeight: '800', padding: '4px 10px', borderRadius: '12px', boxShadow: '0 4px 10px rgba(239, 68, 68, 0.3)', animation: 'pulse 1.5s infinite' }}>
            ...
            <div style={{ padding: '30px 40px', background: '#fff', borderTop: '1px solid #f0f0f0', position: 'relative' }}>
              {showEmojiPicker && (
                <div style={{ position: 'absolute', bottom: '90px', left: '40px', background: 'white', padding: '10px', borderRadius: '12px', boxShadow: '0 10px 25px rgba(0,0,0,0.1)', border: '1px solid #eee', display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: '8px', zIndex: 10 }}>
                  {emojis.map(e => (
                    <button key={e} onClick={() => setReply(r => r + e)} style={{ background: 'none', border: 'none', fontSize: '20px', cursor: 'pointer', padding: '5px', borderRadius: '8px' }}>{e}</button>
                  ))}
                </div>
              )}
              <div style={{ display: 'flex', alignItems: 'center', background: '#f8f7f6', borderRadius: '20px', padding: '8px 10px', border: '1px solid #eee' }}>
                ...
                <input placeholder="Write a message..." style={{ flex: 1, border: 'none', background: 'transparent', outline: 'none', fontSize: '15px', padding: '0 15px' }} value={reply} onChange={(e) => setReply(e.target.value)} onKeyPress={(e) => { if(e.key === 'Enter') handleReply(); }} />
