...
//  CHANGES: Push notification toggle button added
//           (Baaki sab existing code same hai)
//  SEARCH: "PUSH NOTIFICATION" comments dhundo for new additions
// ═══════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════
//  PUSH NOTIFICATION — BACKEND URL
//  (Tumhara Railway backend URL)
// ═══════════════════════════════════════════════════════════
const BACKEND_URL = "https://talksy-production-5d43.up.railway.app";

// ═══════════════════════════════════════════════════════════
//  PUSH NOTIFICATION — HELPER FUNCTIONS
//  (Component ke bahar, top level pe)
// ═══════════════════════════════════════════════════════════
...
async function unsubscribeFromPush(shop) {
  try {
    ...
    console.log('[Push] Unsubscribed for shop:', shop);
    return true;
...
export default function NeuralChatAdmin() {
    ...
    try {
      if (pushEnabled) {
        ...
        if (ok) {
          ...
          alert('🔕 Push notifications band kar di gayi.');
        }
      } else {
        const sub = await subscribeToPush(currentShop);
        if (sub) {
          setPushEnabled(true);
          alert('✅ Push notifications chalu ho gayi!\n\nAb jab bhi koi customer message karega, tumhare browser mein notification aayega — chahe Shopify admin open ho ya nahi.');
        }
      }
    } finally {
  ...
  useEffect(() => {
    ...
    // ✅ CHECK PUSH STATUS ON LOAD — NEW
    checkPushStatus().then((isEnabled) => {
      setPushEnabled(isEnabled);
      console.log('[Push] Status on load:', isEnabled ? 'ON' : 'OFF');
    });
  }, []);

  // Session polling (same as before)
  useEffect(() => {
    const interval = setInterval(async () => {
      try {
        ...
        if (activeSession) {
          ...
          if (updatedActive && updatedActive.shouldBlur && !activeSession.shouldBlur) {
    ...
    const interval = setInterval(async () => {
      try {
        ...
        if (hasNew || hasUpdated) {
          ...
          if (latest && latest.sender === "user" && latest.id !== lastMessageIdRef.current && !isFirstLoadRef.current) {
        ...
        console.error("Message poll error:", err.message);
      }
    }, 1500);
    return () => clearInterval(interval);
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', backgroundColor: '#f9fafb', color: '#111827', fontFamily: '"Inter", system-ui, sans-serif' }}>
      ...
      <div style={{ width: '360px', borderRight: '1px solid #e5e7eb', display: 'flex', flexDirection: 'column', background: '#fff' }}>
        ...
        {/* ✅ SIDEBAR HEADER — Push button add kiya */}
        <div style={{ padding: '20px 24px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div>
            ...
            <p style={{ fontSize: '13px', color: '#6b7280', marginTop: 4, marginBottom: 0 }}>Manage customer conversations</p>
          </div>
          ...
          <button
            ...
            title={pushEnabled ? 'Push notifications band karo' : 'Push notifications chalu karo — customer message kare toh notify ho'}
            style={{
              marginTop: '4px',
              padding: '8px 12px',
              borderRadius: '10px',
              border: pushEnabled ? '2px solid #10b981' : '2px solid #e5e7eb',
              background: pushEnabled
                ? 'linear-gradient(135deg, #d1fae5, #a7f3d0)'
                : '#f9fafb',
              color: pushEnabled ? '#065f46' : '#6b7280',
              fontWeight: '700',
              fontSize: '12px',
              ...
              whiteSpace: 'nowrap',
              flexShrink: 0,
              boxShadow: pushEnabled ? '0 2px 8px rgba(16,185,129,0.2)' : 'none'
            }}
          >
            {pushLoading
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '0 12px' }}>
          {filteredSessions.map(session => (
            <div
              ...
              style={{ position: 'relative', padding: '12px', borderRadius: '12px', cursor: 'pointer', marginBottom: '6px', background: activeSession?.sessionId === session.sessionId ? '#f0f9ff' : 'transparent', border: activeSession?.sessionId === session.sessionId ? '1px solid #bae6fd' : '1px solid transparent', transition: 'all 0.2s', opacity: session.shouldBlur ? 0.6 : 1 }}
            >
            ...
            {!isActiveSessionOverLimit && !isActiveSessionBlurred ? (
              ...
              <div style={{ padding: '20px 32px', background: '#fee2e2', borderTop: '2px solid #ef4444' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '16px', background: 'white', borderRadius: '12px', border: '2px dashed #ef4444' }}>
                  <Icons.EyeOff />
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: '14px', fontWeight: '600', color: '#dc2626', marginBottom: '4px' }}>Chat Expired</div>
                    <div style={{ fontSize: '12px', color: '#dc2626' }}>Upgrade to access full history.</div>
                  </div>
              ...
              <div style={{ padding: '20px 32px', background: '#fef3c7', borderTop: '2px solid #fbbf24' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '16px', background: 'white', borderRadius: '12px', border: '2px dashed #f59e0b' }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: '12px', color: '#92400e' }}>Upgrade to respond to this customer.</div>
                  </div>
